...
plot(women)
fit1 = lm(weight ~ height, data=women) # weight wrt to height
summary(fit1)
range(women$height)
(ndata1=data.frame(height=c(59.5, 62.5))) # only between the range cause only interpolate and no extrapolate
(predicted1 = predict(fit1, newdata= ndata1))
cbind(ndata1, predicted1)
plot(fit1) #check for assumptions in linear regression online
par(mfrow=c(2,2))
plot(fit1)
par(mfrow=c(1,1))
