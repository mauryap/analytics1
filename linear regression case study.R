...
(R= df$Y - fitted(fit1))
plot(density(R)) # to check normal distributed resiuduals or error.
...
summary(fit1)
coef(fit1)
(Y = coef(fit1)[1] + coef(fit1)[2] * 4)# made the equation to make it dynamic by using the coefficient posistion.
